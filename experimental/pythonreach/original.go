...
func moduleFunctionFinder(file *os.File) []*ModuleInfo {
	...
	for scanner.Scan() {
		...
		for module := range moduleInfos {
			...
			if len(matches) > 0 {
				moduleInfos[module].Functions = append(moduleInfos[module].Functions, &FunctionInfo{Name: matches})
			}
		}
	}
	...
	for scanner.Scan() {
		...
		if strings.Contains(scanner.Text(), "==") {
			...
			for _, module := range moduleInfos {
				if strings.ToLower(module.Name) == strings.ToLower(part[0]) {
					module.Name = part[0]
...
func getFilesInDirectory(moduleInfo *ModuleInfo) error {
	...
	if err != nil {
		return fmt.Errorf("module folder not found")
	}
